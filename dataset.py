...
class PairedDatasetMeta(PairedDataset):
    ...
    def from_config(config):
        ...
        # files_a, files_b = map(lambda x: sorted(glob(config[x], recursive=True)), ('files_a', 'files_b'))
        files_a, files_b = [], []
        dataset_base_path = "./GOPRO_Large"

        f = open(config['files_a'])
        lines = f.readlines()
        for line in lines:
            obj = json.loads(line)
            files_a += [os.path.join(dataset_base_path, obj['blur'])]
            files_b += [os.path.join(dataset_base_path, obj['clean'])]
        files_a, files_b = map(tuple, (files_a, files_b))
        # 这两个列表储存的是数据的路径
