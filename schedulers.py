...
class CosineDecay(lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        ...
        return [self.min_lr + (base_lr - self.min_lr) * 0.5 * (1. + math.cos(math.pi *
            (self.last_epoch - self.start_epoch) / (self.num_epochs - self.start_epoch))) for base_lr in self.base_lrs]
