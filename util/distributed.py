...
def slurm_dist_init(backend='nccl'):
    port = "23334"
...
def slurm_dist_init_detectron2(backend='nccl'):
    slurm_dist_init(backend)
    try:
        import numpy as np
        from detectron2.utils import comm
    except Exception as e:
        logger.error(f"Please install detectron2 first")
        raise e
    ...
    groups = []
    rank_list = np.split(np.arange(world_size), num_machines)
    rank_list = [list(map(int, x)) for x in rank_list]
    for i in range(num_machines):
        groups.append(dist.new_group(ranks=rank_list[i]))
    comm._LOCAL_PROCESS_GROUP = groups[rank // num_gpus_per_machine]
