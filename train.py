...
from dataset_long import PairedDataset, PairedDatasetMeta
from metric_counter import MetricCounter
from models.losses import get_loss
from models.models import get_model
from models.networks import get_nets
from schedulers import LinearDecay, WarmRestart, CosineDecay
from fire import Fire
import util.distributed as dist
...
class Trainer:
    ...
    def _run_epoch(self, epoch):
        ...
        if dist.is_master_process():
            ...
            #tq = tqdm.tqdm(self.train_dataset, total=epoch_size, ncols=10)
            
            tq = self.train_dataset
            #tq.set_description('Epoch {}, lr {}'.format(epoch, lr))
        else:
            tq = self.train_dataset
        i = 0
        for data in tq: 
            inputs, targets = self.model.get_input(data)  # return blur images for inputs, and sharp images for targes
            inputs, targets = inputs.to(f'cuda:{self.netG.device_ids[0]}'), targets.to(
                f'cuda:{self.netG.device_ids[0]}')  # put the source into the main gpu 
    ...
    def _validate(self, epoch):
        ...
        if dist.is_master_process():
            self.val_dataset.sampler.set_epoch(epoch)
            tq = self.val_dataset
    ...
    def _get_optim(self, params):
        if self.config['optimizer']['name'] == 'adam':
            ...
            optimizer = optim.AdamW(params, lr=self.config['optimizer']['lr'], weight_decay=0.05, betas=(0.9, 0.95))
    ...
    def _get_scheduler(self, optimizer):
        if self.config['scheduler']['name'] == 'plateau':
            ...
            scheduler = CosineDecay(optimizer,
                                    min_lr=self.config['scheduler']['min_lr'],
                                    num_epochs=self.config['num_epochs'],
                                    start_epoch=self.config['scheduler']['start_epoch'])
        else:
    ...
    def _init_params(self):
        ...
        if dist.get_world_size() > 1:
            # self.criterionG.to(device)
            self.netG.to(device)
            # self.adv_trainer.to(device)
            dist.synchronize()

            ddp = partial(torch.nn.parallel.DistributedDataParallel, device_ids=[device], find_unused_parameters=True)
            # self.criterionG = ddp(self.criterionG)
            # criterionG_without_ddp = self.criterionG.module
            self.netG = ddp(self.netG)
            netG_without_ddp = self.netG.module

            # self.adv_trainer = ddp(self.adv_trainer)
            # adv_trainer_without_ddp = self.adv_trainer.module
            netD = {'patch': ddp(netD['patch'].to(device)), 'full': ddp(netD['full'].to(device))}


        else:
            netG_without_ddp = self.netG

        self.adv_trainer = self._get_adversarial_trainer(self.config['model']['d_name'], netD, criterionD)
        self.model = get_model(self.config['model'])
        self.optimizer_G = self._get_optim(filter(lambda p: p.requires_grad, netG_without_ddp.parameters()))
        self.optimizer_D = self._get_optim(self.adv_trainer.get_params())
        self.scheduler_G = self._get_scheduler(self.optimizer_G)
        self.scheduler_D = self._get_scheduler(self.optimizer_D)


def main(config_path='config/config_full.yaml'):
    with open(config_path, 'r', encoding='utf-8') as f:  # 获得字符串形式的yaml文件内容。
        config = yaml.load(f, Loader=yaml.SafeLoader)  # yaml.load可以把字符串格式的yaml文件转成字典格式
    ...
    data = list(datasets)
    datasets = list(map(PairedDatasetMeta.from_config, data))

    samplers = list(map(partial(DistributedSampler, shuffle=True), datasets))

    get_dataloader = partial(DataLoader,
                             batch_size=batch_size // dist.get_world_size(),
                             shuffle=False, drop_last=True, num_workers=4)
    train, val = (get_dataloader(dataset=datasets[i], sampler=samplers[i]) for i in [0, 1])
    trainer = Trainer(config, train=train, val=val)
    trainer.train()


if __name__ == '__main__':
    dist.slurm_dist_init()

    Fire(main)  # 将main函数暴露在命令行，也就是main函数的输入可以从命令行给定
